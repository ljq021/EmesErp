...
namespace Emes.CodeGen.Template.T4TemplateEngineHost
{
    /// <summary>
    /// T4模板引擎基类
    /// </summary>
    ...
    public abstract class HostBase
    {
        ...
        public HostBase()
        {
            AddAssemblyLocation(typeof(HostBase).Assembly.Location);    //CodeGen.exe
            ...
            AddNamespace("Emes.CodeGen");
            AddNamespace("Emes.CodeGen.Core");

            AddNamespace("Emes.CodeGen.Template.T4TemplateEngineHost");
        ...
        {
            if (assemblyLocationList.Contains(assemblyLocation))
                return;
