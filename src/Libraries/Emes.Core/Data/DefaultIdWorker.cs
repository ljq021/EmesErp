...
namespace Emes.Core.Data
{
    ...
    /// </summary>
    public class DefaultIdWorker : IIdWorker
    {
        //基准时间
        ...
        private readonly EmesOptions _option;
        public DefaultIdWorker()
        {
            var section = Surging.Core.CPlatform.AppConfig.GetSection("Emes");
            if (section.Exists())
                _option = section.Get<EmesOptions>();
            _option = new EmesOptions();

            long workerId = _option.WorkerId;
            long datacenterId = _option.DataCenterId;
            long sequence = _option.Sequence;
            // 如果超出范围就抛出异常
